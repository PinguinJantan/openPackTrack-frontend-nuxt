  ...
  <v-dialog v-model="show" persistent max-width="500px">
      <v-card>
        ...
        <v-card-text class="text-xs-center">
          <v-progress-circular v-if="isSaving" indeterminate :width="3" color="green"></v-progress-circular>
          <input v-if="isInitial || isFailed" type="file" 
...
export default {
  ...
  methods: {
    ...
    upload(formData) {
      ...
      this.$axios.$post('/api/item/import', formData).then(res => {
        if (res.success) {
